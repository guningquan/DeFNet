...
class BimanualEnv(FlexEnv):
    ...
    def _get_obs(self):
        ...
        #     print('the max of depth is',depth.max())
            depth = depth*255.0
            depth = depth.astype(np.uint8)
            # print('the size of depth is',depth.shape)
            depth_st = np.dstack([depth, depth, depth])
            # print('the size of depth is', depth_st.shape)
            depth_img = Image.fromarray(depth_st)
            # print('the size of depth is', depth_img.shape)
    ...
    def _step(self, action, pickplace=False, on_table=True):
        ...
        # print('the grasp height is',place_pos_1[1])
        sub_actions = [
