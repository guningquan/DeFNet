...
from fabricflownet_depth.flownet.models import FlowNet
from fabricflownet_depth.picknet.models import FlowPickSplitModel
from fabricflownet_depth.picknet.dataset import Goals
...
class EnvRollout(object):
    ...
    def load_model(self, load_iter=0):
        ...
        checkpt = torch.load(f'{os.path.dirname(__file__)}/data_depth/{picknet_cfg.flow}')
        ...
        for goal_idx, b in enumerate(self.goal_loader):
            for step in range(len(b)):
                ...
                for rep in range(self.args.goal_repeat):
                    ...
                    print('the size of curr_im is',curr_im.shape)
                    print('the size of goal_im is',goal_im.shape)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--single_pick_thresh', help='min px distance to switch dual pick to single pick',type=int, default=300)
