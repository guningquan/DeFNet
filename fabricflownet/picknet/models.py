...
class FlowPickSplitModel(pl.LightningModule):
    ...
    def nearest_to_mask(self, u, v, depth):
        # print('the max of depth[0,:,:] is', depth[0,:,:].max())
        # print('the max of depth[1,:,:] is', depth[1,:,:].max())
        # print('the max of depth[2,:,:] is', depth[2,:,:].max())
        # print('the max of depth[3,:,:] is', depth[3,:,:].max())
        # print((depth[3,:,:]==depth[1,:,:]).all())
        depth=(depth[0,:,:]+depth[1,:,:]+depth[2,:,:]+depth[3,:,:])/3
        print('the size of depth is', depth.shape)
        mask_idx = np.argwhere(depth)
        print('the u is',u)
        print('the v is',v)
        print('the size of mask_idx is',mask_idx.shape)
        nearest_idx = mask_idx[((mask_idx - [u,v])**2).sum(1).argmin()]
        return nearest_idx  # 2 dimensional
    ...
    def get_action(self, flow, depth_pre, depth_post):
        ...
        print('## the size of depth_pre is', depth_pre.shape)
        depth_pre_np = depth_pre.detach().squeeze().cpu().numpy()
        print('## the size of depth_pre is', depth_pre_np.shape)
        # print('the size of pick_uv1[0] is ',pick_uv1[0].shape)
        # print('the size of pick_uv1[1] is ',pick_uv1[1].shape)
        print('the size of depth_pre_np is ',depth_pre_np.shape)
        ...
        # print('the pick_thres is',self.s_pick_thres)
    ...
    def forward(self, flow, depth_pre, depth_post):
        ...
        ###############################################################
        if flow.size()[0]!=1:
            pick1_gar_new=pick1_gau
            for i in range(flow.size()[0]-1):
                pick1_gar_new=torch.cat((pick1_gar_new,pick1_gau),dim=0)
            pick1_gau=pick1_gar_new
        ###############################################################
        if self.input_mode == 'flowonly':
            # print('the flow size is ',flow.size())
            # print('the pick1_gau size is ',pick1_gau.size())
    ...
    def get_gaussian(self, u, v, sigma=5, size=None):
        if np.isscalar(u):
            # print('the scalar gaussian is runing')
            if size is None:
                size = self.im_width
            ...
            g = g.unsqueeze(1)
            # print('this is the end of gassus scat')

        else:
            if len(u)==1:
                # print('gassus short tensor start')
                if size is None:
                    size = self.im_width
                # print('the u is ',u)
                x0, y0 = torch.Tensor([u]).cuda(), torch.Tensor([v]).cuda()
                ...
                g = (g - gmin[:, None, None]) / (gmax[:, None, None] - gmin[:, None, None])
                g = g.unsqueeze(1)
                # print('end of short tensro gassi')


            else:
                # print('gassus long tensor start')
                if size is None:
                    size = self.im_width
                # print('the u is ', u)
                N = u.shape[0]
                # x0, y0 = torch.Tensor(u).cpu(), torch.Tensor(v).cpu()
                x0, y0 = u,v
                x0 = x0[:, None]
                y0 = y0[:, None]
                # sigma = torch.tensor(sigma, dtype=torch.float32, device=self.device)
                ...
                g = (g - gmin[:, None, None]) / (gmax[:, None, None] - gmin[:, None, None])
                g = g.unsqueeze(1)
                # print('end of long tensro gassi')

        return g


    def loss(self, logits1, logits2, pick_uv1, pick_uv2,batch_size):
        if batch_size==1:
            N = logits1.size(0)
            W = logits1.size(2)
            ...
            if self.min_loss:
                loss_1a = torch.mean(F.binary_cross_entropy_with_logits(logits1, label_a, reduction='none'),
                                     dim=(1, 2, 3))
                loss_1b = torch.mean(F.binary_cross_entropy_with_logits(logits1, label_b, reduction='none'),
                                     dim=(1, 2, 3))
                loss_2a = torch.mean(F.binary_cross_entropy_with_logits(logits2, label_a, reduction='none'),
                                     dim=(1, 2, 3))
                loss_2b = torch.mean(F.binary_cross_entropy_with_logits(logits2, label_b, reduction='none'),
                                     dim=(1, 2, 3))
    ...
    def training_step(self, batch, batch_idx):
        # print("the training function is running")
        opt1, opt2 = self.optimizers()
        depth_pre, depth_post, flow, pick_uv1, pick_uv2 = batch
        # print('the training flow size is',flow.size())
        if self.input_mode == 'flowonly':
            x1 = flow
        else:
            raise NotImplementedError
        ...
        loss1, loss2 = self.loss(info['logits1'], info['logits2'], pick_uv1, pick_uv2,flow.shape[0])
        ...
        torch.save(self.first.state_dict(), f'/home/heruhan/gnq/paper_codes/FabricFlowNet/outputs/first.pt')
        torch.save(self.second.state_dict(), f'/home/heruhan/gnq/paper_codes/FabricFlowNet/outputs/second.pt')

        ###################################################################################


        # print("the end of training function")

    def validation_step(self, batch, batch_idx, log=True):
        depth_pre, depth_post, flow, pick_uv1, pick_uv2 = batch
        # print("the val function is running")
        # print('the val flow size is',flow.size())
        if self.input_mode == 'flowonly':
            x1 = flow
        else:
            raise NotImplementedError
        ...
        uv1, uv2, info = self.forward(x1, depth_pre, depth_post)
        # print('the shape of flow.shape[0] is', flow.shape[0])
        loss1, loss2 = self.loss(info['logits1'], info['logits2'], pick_uv1, pick_uv2,flow.shape[0])
    ...
    def plot(self, batch, pred_uv1, pred_uv2, info, stage):
        ...
        ax[0][0].imshow(depth_post.transpose(1,2,0)[:][:][:-1])
        # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # #
