...
class PickNetDataset(Dataset):
    def __init__(self, camera_params, config, ids, mode='train', pick_pt=True):
        ...
        if self.cfg.flow == 'gt':
            ...
            self.gt_flow = False
            self.flow = FlowNet(input_channels=8)
    ...
    def __getitem__(self, index):
        id = self.ids[index]
        # print('the running id is',id)
        action = np.load(f'{self.data_path}/actions/{id}_actions.npy')
        pick_uv1 = action[0]
        place_uv1 = action[1]
        pick_uv2 = action[2]
        place_uv2 = action[3]
        ...
        if self.gt_flow:
            ...
            # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # #
            img_pre = cv2.imread(f'{self.data_path}/images/{id}_rgb_before.png') / 255.0
            depth_pre = np.concatenate((img_pre, depth_pre[:, :, None]), axis=-1)
            img_post = cv2.imread(f'{self.data_path}/images/{id}_rgb_after.png') / 255.0
            depth_post = np.concatenate((img_post, depth_post[:, :, None]), axis=-1)
            # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # #
            ...
            if not osp.exists(osp.join(self.data_path, "flow_pred", f"{id}_flow.npy")):
                ...
                inp = torch.cat([torch.FloatTensor(depth_pre), torch.FloatTensor(depth_post)], -1).permute(2, 0,1).unsqueeze(0)
                # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # #
                # print('1. the size of inp is', inp.shape)
                flow_im = self.flow(inp)
                # print('2. the size of flow_im is',flow_im.shape)
        ...
        img_post = cv2.imread(f'{self.data_path}/images/{id}_rgb_after.png') / 255.0
        depth_post = np.concatenate((img_post, depth_post[:, :, None]), axis=-1)
        depth_pre = torch.FloatTensor(depth_pre).permute(2, 0, 1)
        depth_post = torch.FloatTensor(depth_post).permute(2, 0, 1)
        # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # #
        ...
        # mask flow
        # print('3. the size of flow_im is',flow_im.shape)
        flow_im[0,:,:][depth_pre[0] == 0] = 0
        flow_im[1,:,:][depth_pre[0] == 0] = 0
        # flow_im[2,:,:][depth_pre[0] == 0] = 0
        # flow_im[3,:,:][depth_pre[0] == 0] = 0
        # flow_im[4,:,:][depth_pre[0] == 0] = 0
        # flow_im[5,:,:][depth_pre[0] == 0] = 0
        # flow_im[6,:,:][depth_pre[0] == 0] = 0
        # flow_im[7,:,:][depth_pre[0] == 0] = 0
...
class Goals(Dataset):
    def __init__(self, cloth_type='square_towel'):
        ...
        for goal_name in goal_names:
            ...
            goal_im_depth = cv2.imread(f'{multistep_path}/depth/{goal_name}_depth.png')[:, :, 0] / 255  #only one dimensional
            # goal_im_rgb = cv2.imread(f'{multistep_path}/rgb/{goal_name}.png') / 255
            # goal_im=np.concatenate((goal_im_rgb[:,:,:],goal_im_depth[:,:,None]),axis=-1)
            goal_im=np.load(f'{multistep_path}/rgbd/{goal_name}.npy') / 255
            # goal_im = np.concatenate((goal_im[:, :, None], goal_im[:, :, None], goal_im[:, :, None], goal_im[:, :, None]),axis=-1)
